...
** Last update Thu Dec 17 03:03:12 2015 HOUVET Benoît - Xavier
*/
...
{
  my_putstr(ennemi->name);
  my_putstr(" vous a capturé, vous avez donc perdu ");
  my_putstr(player->name);
  my_putstr(" de votre team.\n");
}

void	print_fail_capture_ennemi(t_creature *ennemi)
{
  my_putstr(ennemi->name);
  my_putstr(" a essayé de vous capturer mais, il a échoué\n");
}
